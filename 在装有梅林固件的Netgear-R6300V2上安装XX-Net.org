...
5. 下载Entware-NG脚本 百度网盘：http://pan.baidu.com/s/1dD19lct ，Google Driver：https://drive.google.com/file/d/0B1SYsraWbW-rTlhLX0FLZUZwNWs/view?usp=sharing 。然后用WINSCP上传到到路由器的/tmp/目录下
6. 输入命令
    #+BEGIN_EXAMPLE 
    chmod +x /tmp/entware.ng-setup.sh&&/bin/sh /tmp/entware_install.sh
    #+END_EXAMPLE
    如果没有出现什么异常应该会显示
    #+BEGIN_EXAMPLE
...
6. 现在我们开始安装XX-net需要的python库，输入
    ...
    #+END_EXAMPLE
7. 下载XXnet，解压到/opt/XX-net，编辑/XX-Net/Launcher/start.py 找到import webbrowser这行，前面加上“#”把这行注释掉。因为Entware是给路由器设计的，安装的Python没有Webrowser这个库。我已经提交了Patch把这一行代码放在host_port = config.get(["modules", "launcher", "control_port"], 8085)这行后面。但是还没有更新到最新的版本里面。如果嫌更新之后又要去改文件麻烦，可以把\XX-Net\python27\1.0\lib\webbrowser.py 拷贝到\opt\lib\python2.7文件夹里面。
8. 尝试运行一次，输入
    #+BEGIN_EXAMPLE
...
更新于2015-11-28：将Entware-ARM升级到Entware-NG. 对原Entware-ARM的安装脚本做了编辑，修改了部分Bug和地址。
